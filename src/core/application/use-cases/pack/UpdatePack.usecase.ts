import { PackUpdateData } from "@/core/domain/port/pack/PackRepository";
...
export class UpdatePackUseCase {
  ...
  async execute(
    token: string,
    args: { packId: string; pack: PackUpdateData },
  ): Promise<void> {
    ...
    await packRepository.update(args.packId, args.pack);
  }
}
